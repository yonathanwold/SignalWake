...
const details: Record<string, { title: string; kicker: string; body: string; next: string }> = {
  ...
  health: { title: "System Health", kicker: "SOURCE FRESHNESS", body: "Runtime source freshness, API behavior, and persisted processing runs.", next: "Inspect API health" },
};
...
export default async function FutureSurfacePage({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (section === "health") return <HealthWorkspace />;
