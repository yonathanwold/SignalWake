...
import type { HealthMetrics, HealthSource, HealthSummary } from "../lib/types";
...
export function HealthWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMetrics(body);
      setReady(readyResponse.ok && readyBody.status === "ready" ? "ready" : "not ready");
    } catch (cause) {
      setHealth(null);
      setMetrics(null);
  ...
  const platformState = health?.overall_state || "UNKNOWN";
  const databaseState = health?.database_state || "disconnected";
  ...
  return <div className="health-page">
    ...
    <div className="health-overview"><div className="health-overall"><span className={stateClass(platformState)} /><div><small>PLATFORM STATE</small><strong>{platformState}</strong><p>Database <b>{databaseState.toUpperCase()}</b> · readiness <b>{ready.toUpperCase()}</b> · API health summary</p></div></div><div className="health-state-counts"><span>ACTIVE <b>{stateCounts.ACTIVE || 0}</b></span><span>DEGRADED <b>{stateCounts.DEGRADED || 0}</b></span><span>DOWN <b>{stateCounts.DOWN || 0}</b></span><span>UNKNOWN <b>{stateCounts.UNKNOWN || 0}</b></span></div></div>
