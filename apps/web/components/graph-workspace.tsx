...
import type { GraphEdge, GraphNode, GraphNodeList, GraphSubgraph } from "../lib/types";
...
export function GraphWorkspace() {
  ...
  return <div className="graph-page">
    ...
    <div className="graph-toolbar" aria-label="Graph filters"><label><span>ASSET TYPE</span><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option>ALL</option><option value="port">PORT</option><option value="rail_corridor">RAIL CORRIDOR</option></select></label><label><span>REGION</span><select value={regionFilter} onChange={(event) => setRegionFilter(event.target.value)}>{regions.map((region) => <option key={region}>{region}</option>)}</select></label><label><span>RELATIONSHIP</span><select value={relationshipFilter} onChange={(event) => setRelationshipFilter(event.target.value as (typeof relationOptions)[number])}>{relationOptions.map((relation) => <option key={relation}>{relation.replaceAll("_", " ")}</option>)}</select></label><span className="graph-toolbar-note">DERIVED EDGES ONLY · NO NATIONAL GRAPH DEFAULT</span></div>
