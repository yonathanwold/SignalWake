...
The USGS adapter uses the authoritative rolling past-day GeoJSON summary feed. The API keeps global operational event pages bounded at 4,000 records inside the past 48-hour UTC contract, and the web map requests that maximum so the live view is not truncated; it does not download unbounded history. Individual source adapters retain lower caps where needed. Point observations are clustered at national zoom while retaining source coordinates. Road511 is optional and never requests without `ROAD511_API_KEY`. Open-Meteo (capped multi-coordinate model fields), NPPES (one-state provider references), and Census TIGERweb (capped state polygons) are request-scoped layer adapters, not event ingestion. RainViewer has a separate metadata contract and raster source; no radar pixel is converted into a marker.
...
The Operational Map mounts MapLibre GL JS with local worker assets, a public token-free CARTO `dark_all` OpenStreetMap raster basemap, and a token-free OpenStreetMap Standard raster failover, plus GeoJSON sources for canonical event points, polygons, and infrastructure reference assets. The map is centered on the geographic United States at `[-98.5795, 39.8283]` and starts at zoom `4`. Source geometry is passed to MapLibre unchanged; latitude/longitude is used only when an API record has no geometry. MapLibre's visible attribution names the active OpenStreetMap/CARTO provider. Public tile availability and rate limits apply. The runtime tries CARTO first, then OpenStreetMap Standard on basemap source/tile failure; only after all real providers fail does it report `BASEMAP UNAVAILABLE`, and it never renders a synthetic map fallback.
